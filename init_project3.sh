...
if curl -s http://169.254.169.254/latest/dynamic/instance-identity/ > /dev/null
then
    echo "Running on an AWS EC2 instance. Running ec2_deps.sh..."
    ...
    echo "Not running on an AWS EC2 instance. ec2_deps.sh will not be executed."
fi
...
then
    ...
    then
        ...
        echo "AWS CLI installation is completed."
    else
